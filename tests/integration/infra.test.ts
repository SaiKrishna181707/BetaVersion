...
test('CDK defines main storage schema, protected API, cross-account worker and shared limits', () => {
  const app = new App();
  ...
  control.hasResourceProperties('AWS::ApiGatewayV2::Route', { RouteKey: 'ANY /{proxy+}', AuthorizationType: 'JWT', AuthorizationScopes: ['openid'] });
  control.hasResourceProperties('AWS::Cognito::UserPoolClient', { GenerateSecret: false, AllowedOAuthFlows: ['code'] });
