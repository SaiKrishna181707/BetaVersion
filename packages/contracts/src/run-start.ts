/**
 * Accepting a run lives in the contracts package for the same reason planning does: the local
 * runtime and the AWS runtime must persist exactly the same record before anything executes.
 * `services/agent-worker/src/run-start.ts` re-exports it, so no importer changed.
 */
import { runLimitsFromConfiguration } from './limits';
import type { ExecutionMode, RunRecord, RunStartInput, RunStartResponse } from './run';
