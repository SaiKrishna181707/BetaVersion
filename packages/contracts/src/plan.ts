/**
 * Run planning lives in the contracts package because both sides of the execution boundary
 * need it: the control plane plans a run before it starts a state machine, and the local
 * orchestrator plans the same run before it opens a browser. One implementation, so "twenty
 * synthetic users" cannot mean two different things depending on where the run executes.
 *
 * The behaviour and comments are those of the original `services/agent-worker/src/run-plan.ts`,
 * which now re-exports this file so no existing importer changed.
 */
import { GUARDRAILS } from './model';
import { runLimitsFromConfiguration } from './limits';
import type { RunConfiguration } from './model';
import type { RunExecutionLimits } from './run';
import type { RunPlan, SessionPlan } from './execution';
