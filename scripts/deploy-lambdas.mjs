...
async function main() {
  ...
  const workerInfo = await deployCode('synthetic-beta-session-worker', '.artifacts/lambda-bundles/worker');

  console.log('\n[2/3] Deploying finalizer Lambda...');
  const finalizerInfo = await deployCode('synthetic-beta-finalizer', '.artifacts/lambda-bundles/finalizer');

  console.log('\n[3/3] Deploying API Lambda...');
  const apiInfo = await deployCode('synthetic-beta-api', '.artifacts/lambda-bundles/api');

  if (commitSha) {
    await updateApiEnvironment('synthetic-beta-api', commitSha);
  }

  console.log('\n========================================');
  console.log('DEPLOYMENT SUMMARY:');
  console.log(`synthetic-beta-session-worker: CodeSha256=${workerInfo.CodeSha256} LastModified=${workerInfo.LastModified}`);
  console.log(`synthetic-beta-finalizer:      CodeSha256=${finalizerInfo.CodeSha256} LastModified=${finalizerInfo.LastModified}`);
  console.log(`synthetic-beta-api:            CodeSha256=${apiInfo.CodeSha256} LastModified=${apiInfo.LastModified}`);
