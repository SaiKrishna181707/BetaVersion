...
export function RunPage({ runId }: { runId: string }) {
  ...
  return <WorkspaceShell crumb={`Run ${run.run_id}`} footerNote="Outcomes are calculated from recorded evidence. Cost allowances and estimates are labelled separately." status={<Badge tone={run.state === 'COMPLETED' ? 'accent' : run.state === 'RUNNING' || run.state === 'QUEUED' ? 'neutral' : 'warning'}><span className="dot" />{run.state}</Badge>}>
    ...
    <div className="stat-grid"><Stat label="Sessions finished" value={`${run.finished_session_count}/${run.session_count}`} detail={`${progress}% of the cohort`} /><Stat label={run.mode === 'AWS' ? 'Execution cost estimate' : 'Recorded action allowance'} value={formatUsd(run.spent_cents)} detail={`budget ${formatUsd(run.budget_cents)}${run.mode === 'AWS' ? ' · AWS billing may differ' : ''}`} /><Stat label="Started" value={formatInstant(run.started_at)} /><Stat label="Finished" value={formatInstant(run.finished_at)} /></div>
